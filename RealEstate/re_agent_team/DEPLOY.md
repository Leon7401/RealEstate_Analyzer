...
- スクレイピングは `playwright install chromium` 済み環境で、拒否時にブラウザ取得へ自動切替します（`SCRAPE_USE_BROWSER=0` で無効化可）
- 楽待など Cloudflare 配下のサイトは、クラウド/データセンターIPだと一覧が403になることがあります。その場合は連合隊ソースか、自宅回線での実行を使ってください
